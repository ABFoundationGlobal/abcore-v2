	...
	for {
		...
		case cHead := <-voteManager.highestVerifiedBlockCh:
			...
			if ok, sourceNumber, sourceHash := voteManager.UnderRules(curHead); ok {
				...
				switch e := voteManager.engine.(type) {
				case *parlia.Parlia:
					innerParlia = e
				...
				if innerParlia != nil {
					...
					blockInterval, err := innerParlia.BlockInterval(voteManager.chain, curHead)
					if err != nil {
						log.Debug("failed to get BlockInterval when voting")
					}
					voteAssembledTime := time.UnixMilli(int64((curHead.MilliTimestamp() + innerParlia.GetAncestorGenerationDepth(curHead)*blockInterval)))
