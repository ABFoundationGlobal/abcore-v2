...
func (p *BlobPool) Reset(oldHead, newHead *types.Header) {
	...
	if !p.chain.Config().IsOsaka(oldHead.Number, oldHead.Time) && p.chain.Config().IsOsaka(newHead.Number, newHead.Time) && !p.chain.Config().IsParliaActive(newHead.Number) {
...
func (p *BlobPool) reinject(addr common.Address, txhash common.Hash) error {
	...
	head := p.head.Load()
	if p.chain.Config().IsOsaka(head.Number, head.Time) && tx.BlobTxSidecar().Version == types.BlobSidecarVersion0 && !p.chain.Config().IsParliaActive(head.Number) {
		if err := tx.BlobTxSidecar().ToV1(); err != nil {
