#!/usr/bin/env bash
# Show status of DevNet nodes on the current machine.
#
# Usage:
#   ./status-single.sh [node...]
#
# If no nodes are specified, shows all nodes that have a container on this machine.
#
# Examples:
#   ./status-single.sh val-0 val-1   # server-1
#   ./status-single.sh val-4          # server-3
#   ./status-single.sh                # auto-detect all local containers
#
# Environment overrides:
#   DATA_DIR  — node data root (default: ./data)
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
source "$SCRIPT_DIR/lib.sh"

# ---------------------------------------------------------------------------
# Determine which nodes to show
# ---------------------------------------------------------------------------
if [[ $# -gt 0 ]]; then
    for node in "$@"; do
        if ! node_is_valid_name "$node"; then
            error "Unknown node name: '$node'. Valid names: ${VALID_NODES[*]}"
            exit 1
        fi
    done
    NODES=("$@")
else
    # Auto-detect: show nodes that have a container (running or stopped)
    NODES=()
    for node in "${VALID_NODES[@]}"; do
        if container_exists "$node"; then
            NODES+=("$node")
        fi
    done
    if [[ ${#NODES[@]} -eq 0 ]]; then
        info "No devnet containers found on this machine."
        exit 0
    fi
fi

section "DevNet Node Status (this machine)"

printf "%-10s %-14s %-8s %-8s %-8s %s\n" "NODE" "CONTAINER" "BLOCK" "PEERS" "MINING" "IMAGE"
printf "%-10s %-14s %-8s %-8s %-8s %s\n" "----" "---------" "-----" "-----" "------" "-----"

for node in "${NODES[@]}"; do
    name="$(node_container_name "$node")"
    port="$(node_rpc_port "$node")"

    if ! container_exists "$node"; then
        printf "%-10s %-14s %s\n" "$node" "$name" "(no container)"
        continue
    fi

    running=$(docker inspect -f '{{.State.Running}}' "$name" 2>/dev/null || echo false)
    if [[ "$running" != "true" ]]; then
        status=$(docker inspect -f '{{.State.Status}}' "$name" 2>/dev/null || echo "unknown")
        printf "%-10s %-14s %s\n" "$node" "$name" "(container $status)"
        continue
    fi

    img=$(docker inspect --format '{{.Config.Image}}' "$name" 2>/dev/null | sed 's|.*/||')

    if wait_for_rpc "$port" 3; then
        bn=$(block_number "$port")
        peers=$(peer_count "$port")
        mining_raw=$(rpc_call "$port" eth_mining '[]' \
            | python3 -c "import sys,json; print(str(json.load(sys.stdin).get('result','?')).lower())" 2>/dev/null || echo "?")
        if [[ "$mining_raw" == "true" ]]; then mining="yes"; else mining="no"; fi
        printf "%-10s %-14s %-8s %-8s %-8s %s\n" "$node" "$name" "$bn" "$peers" "$mining" "$img"
    else
        printf "%-10s %-14s %-8s %-8s %-8s %s\n" "$node" "$name" "RPC?" "-" "-" "$img"
    fi
done

# ---------------------------------------------------------------------------
# Recent errors/warnings
# ---------------------------------------------------------------------------
echo ""
echo -e "${BLUE}Recent errors/warnings (last 5 per node):${NC}"
for node in "${NODES[@]}"; do
    name="$(node_container_name "$node")"
    if container_running "$node"; then
        lines=$(docker logs --tail 50 "$name" 2>&1 | grep -iE "err|warn|crit|fatal" | tail -5 || true)
        if [[ -n "$lines" ]]; then
            echo -e "  ${YELLOW}$node:${NC}"
            echo "$lines" | sed 's/^/    /'
        fi
    fi
done

echo ""
echo -e "${BLUE}Useful commands:${NC}"
echo "  Logs (follow)  : docker logs -f devnet-<node>"
echo "  Attach console : docker exec -it devnet-<node> geth attach /data/geth.ipc"
echo "  Stop node      : docker stop devnet-<node> && docker rm devnet-<node>"
echo "  Upgrade        : ./upgrade-single.sh <step 1-5> <node...> <image>"
