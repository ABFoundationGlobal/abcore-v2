...
# Capture whether PORT_BASE was explicitly set before lib.sh applies its default.
...
# ---- Ensure custom geth binary is available ----
if [[ -z "${ABCORE_V2_GETH:-}" ]] && [[ ! -x "${SCRIPT_DIR}/bin/geth-custom" ]]; then
  echo "[$(date +'%H:%M:%S')] bin/geth-custom not found; running 00-build-contracts.sh"
  "${SCRIPT_DIR}/00-build-contracts.sh"
fi
...
run "${SCRIPT_DIR}/02-start-validators.sh"
run "${SCRIPT_DIR}/10-scn1-pre-fork-poa.sh"
run "${SCRIPT_DIR}/20-scn2-poa-to-posa.sh"
run "${SCRIPT_DIR}/30-scn3-block-forks.sh"
run "${SCRIPT_DIR}/40-scn4-time-forks.sh"
