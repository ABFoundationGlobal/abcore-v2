...
export PATH="${HOME}/.local/bin:${HOME}/.foundry/bin:/usr/local/src/go/bin:${HOME}/go/bin:${PATH}"
...
# BLS12-381 G1 compressed public key = 48 bytes (96 hex chars)
BLS_PLACEHOLDER="0x$(printf ab%.0s {1..48})"
...
# NOTE: no header line — the generate-validators script treats every line as a validator.
